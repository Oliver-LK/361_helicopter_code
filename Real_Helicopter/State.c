...
error_t set_desired_pos(error_t desired_pos, heli_state current_heli_state) {
    ...
    case LANDED:
        ...
        desired_pos->alt_desired = ALT_MIN + 10_PERCENT_CHANGE/2; // Causes the PID controller to aim for the minimum altitude.
        break;
    ...
    case FLYING:
        if(button_event(UP) == PUSHED) {
            desired_pos->alt_desired -= 10_PERCENT_CHANGE; // Up button increases altitude
            ...
            desired_pos->alt_desired += 10_PERCENT_CHANGE; // Down button increases altitude
